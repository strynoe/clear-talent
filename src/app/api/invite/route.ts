...
export async function GET(request: Request) {
  try {
    ...
    const res = await fetch(
      supabaseUrl(`invite_links?id=eq.${token}&select=id,type,label,used_at,expires_at&limit=1`),
...
export async function POST(request: Request) {
  try {
    ...
    const res = await fetch(supabaseUrl('invite_links?select=id'), {
      method: 'POST',
    ...
    if (!row?.id) {
      return Response.json({ error: 'No id returned' }, { status: 500 })
    }
